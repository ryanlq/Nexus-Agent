...
export const en: Translations = {
  ...
  loops: {
    title: 'Loops',
    description: 'Recurring agent tasks. Click to view each run’s output.',
    search: 'Search loops…',
    newLoop: 'New loop',
    interval: 'Interval',
    intervalHint: 'e.g. 10m, every 2h, */10 * * * *',
    mode: 'Pace',
    modeFixed: 'Fixed interval',
    modeAgent: 'Agent-paced',
    modeAgentHint: 'No fixed timer. After each run the loop idles until the agent decides to run again (immediately or after a delay) or stop. Best when each run takes an unpredictable amount of time.',
    agentPacedBadge: 'agent-paced',
    idleLabel: 'idle',
    task: 'Task',
    taskPlaceholder: 'What should this loop do each run…',
    name: 'Name (optional)',
    namePlaceholder: 'Name this loop',
    maxRuns: 'Max runs (optional)',
    maxRunsHint: 'Stop after this many runs. Blank = run forever.',
    maxRunsPlaceholder: 'e.g. 5',
    stopCondition: 'Stop condition (optional)',
    stopConditionHint: 'The agent evaluates this each run and stops the loop when true.',
    stopConditionPlaceholder: 'e.g. the deploy is green and stable for 3 checks',
    iterations: (completed: number, max: number | null) =>
      max == null ? `${completed} runs (no cap)` : `${completed} / ${max} runs`,
    ...
    output: 'Output',
    outputOf: (title: string) => `Output of ${title}`,
    noOutput: 'This loop has not run yet.',
    loadingOutput: 'Loading output…',
    runs: (count: number) => `${count} ${count === 1 ? 'run' : 'runs'}`,
    emptyTitle: 'No loops yet',
    emptyDesc: 'Create one with the /loop command, or “New loop” above.',
    refresh: 'Refresh',
    pause: 'Pause',
    resume: 'Resume',
    trigger: 'Run now',
    ...
    failedCreate: 'Failed to create loop.',
    failedLoad: 'Failed to load loops.',
    failedLoadOutput: 'Failed to load output.',
    close: 'Close',
    last: 'Last',
    next: 'Next'
  },
