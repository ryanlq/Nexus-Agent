import type { ConnectionState, GatewayEvent } from '@nexus-agent/desktop-shared'
...
const isOpen = (gateway: NexusGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<NexusGateway | null>(null)
...
let primaryGateway: NexusGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: NexusGateway | null, profile = 'default'): void {
...
export function activeGateway(): NexusGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<NexusGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
