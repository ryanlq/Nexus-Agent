...
async function mediaSrc(path: string): Promise<string> {
  ...
  if (window.nexusAgent && ['audio', 'video'].includes(mediaKind(path))) {
    return mediaStreamUrl(path)
  }

  if (!window.nexusAgent?.readFileDataUrl) {
    return mediaExternalUrl(path)
  }

  return window.nexusAgent.readFileDataUrl(filePathFromMediaPath(path))
}
...
function OpenMediaButton({ kind, path }: { kind: 'audio' | 'video'; path: string }) {
  return (
    <button
      ...
      onClick={() => void window.nexusAgent?.openExternal(mediaExternalUrl(path))}
      type="button"
    >
      Open {kind} file
    </button>
