...
export const zh: Translations = {
  ...
  boot: {
    ready: 'Nexus Agent 已就绪',
    desktopBootFailedWithMessage: message => `桌面启动失败:${message}`,
    steps: {
      connectingGateway: '正在连接实时桌面网关',
      loadingSettings: '正在加载 Nexus Agent 设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingNexusAgent: '正在启动 Nexus Agent…'
    },
    errors: {
      backgroundExited: 'Nexus Agent 后台进程已退出。',
      backgroundExitedDuringStartup: 'Nexus Agent 后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 Nexus Agent 后端早于当前桌面构建,可能无法正常工作。请更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求(405 Method Not Allowed)。请尝试重启 Nexus Agent。',
  ...
  settings: {
    ...
    resetConfirm: '将所有设置恢复为 Nexus Agent 默认值？',
    exportFailed: '导出失败',
    resetFailed: '重置失败',
    nav: {
      gateway: '网关',
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 Nexus Agent 跟随系统设置。',
    ...
    fieldDescriptions: {
      ...
      timezone: '当 Nexus Agent 需要本地时间上下文时使用。留空则使用系统时区。',
      ...
      file_read_max_chars: 'Nexus Agent 单次文件读取可读取的最大字符数。',
      'approvals.mode': 'Nexus Agent 如何处理需要显式审批的命令。',
      ...
      'agent.max_turns': 'Nexus Agent 停止一次运行前工具调用轮次的上限。'
    },
    about: {
      heading: 'Nexus Agent',
      ...
      automaticUpdatesDesc: 'Nexus Agent 会在后台自动检查更新,并在有可用更新时通知你。',
  ...
  messaging: {
    ...
    regionFeishu: '飞书',
    regionLark: 'Lark',
    regionHint: '飞书 = 国内版 (open.feishu.cn);Lark = 国际版 (open.larksuite.com)。自动设置 API 域名。',
    ...
    platformIntro: {
      ...
      signal: '在可访问的位置运行 signal-cli REST 桥接,然后把 Nexus Agent 指向该 URL 和已注册的电话号码。',
      whatsapp: '启动 Nexus Agent 自带的 WhatsApp 桥接,首次运行时扫描二维码,然后启用该平台。',
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server,暴露其 API,然后用服务器密码把 Nexus Agent 指向该 URL。',
  ...
  loops: {
    title: '循环任务',
    description: '周期性运行的 agent 任务。点击查看每次产出。',
    search: '搜索循环任务…',
    newLoop: '新建 Loop',
    interval: '间隔',
    intervalHint: '例:10m、every 2h、*/10 * * * *',
    mode: '节奏',
    modeFixed: '固定间隔',
    modeAgent: 'Agent 自决',
    modeAgentHint: '不设固定计时。每次跑完后 loop 进入闲置,直到 agent 自己决定何时再来一轮(立刻或延时),或自行停止。最适合每次耗时不可预测的任务。',
    agentPacedBadge: 'agent 节奏',
    idleLabel: '闲置',
    task: '任务',
    taskPlaceholder: '这个 loop 每次要做什么…',
    name: '名称(可选)',
    namePlaceholder: '给这个 loop 起个名字',
    maxRuns: '最大次数(可选)',
    maxRunsHint: '运行这么多次后自动停止。留空 = 一直运行。',
    maxRunsPlaceholder: '例:5',
    stopCondition: '停止条件(可选)',
    stopConditionHint: 'agent 每次运行会判断,满足时自行停止该 loop。',
    stopConditionPlaceholder: '例:部署变绿且连续 3 次稳定',
    iterations: (completed: number, max: number | null) =>
      max == null ? `${completed} 次(无上限)` : `${completed} / ${max} 次`,
    stopConditionBadge: '停止条件',
    create: '创建',
    cancel: '取消',
    creating: '创建中…',
    viewOutput: '查看产出',
    output: '产出',
    outputOf: (title: string) => `${title} 的产出`,
    noOutput: '这个 loop 还没有运行过。',
    loadingOutput: '正在加载产出…',
    runs: (count: number) => `${count} 次运行`,
    emptyTitle: '还没有循环任务',
    emptyDesc: '用 /loop 命令,或点上面的"新建 Loop"创建一个。',
    refresh: '刷新',
    pause: '暂停',
    resume: '恢复',
    trigger: '立即运行',
    delete: '删除',
    created: '已创建循环任务。',
    failedCreate: '创建循环任务失败。',
    failedLoad: '加载循环任务失败。',
    failedLoadOutput: '加载产出失败。',
    close: '关闭',
    last: '上次',
    next: '下次'
  },
