...
//   - nexus_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - nexus_session_rt: the OAuth refresh token. Long-lived (24h rotating,
...
const AT_COOKIE_VARIANTS = ['__Host-nexus_session_at', '__Secure-nexus_session_at', 'nexus_session_at']
const RT_COOKIE_VARIANTS = ['__Host-nexus_session_rt', '__Secure-nexus_session_rt', 'nexus_session_rt']
