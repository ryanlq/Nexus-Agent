...
import { getProfiles, setApiRequestProfile } from '@/nexus'
...
import type { ProfileInfo } from '@/types/nexus'
...
// /api/profiles/active `current`). "default" is the root ~/.nexus-agent. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'nexus.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'nexus.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.nexusAgent.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.nexusAgent.profile.set(name)
}
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'nexus.desktop.showAllProfiles'
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.nexusAgent?.touchBackend?.(target).catch(() => undefined)
}
