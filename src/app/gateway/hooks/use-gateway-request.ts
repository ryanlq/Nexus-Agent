...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<NexusGateway | null>(null)

  const connectionRef = useRef<Awaited<ReturnType<NonNullable<typeof window.nexusAgent>['getConnection']>> | null>(
    null
  )

  const gatewayStateRef = useRef(gatewayState)
  const reconnectingRef = useRef<Promise<NexusGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as NexusGateway | null
      }),
    []
  )
