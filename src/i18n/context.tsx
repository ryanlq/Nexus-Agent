...
import { getGatewayConfigRecord, type GatewayConfigRecord, saveGatewayConfig } from '@/nexus'
...
export interface I18nConfigClient {
  getConfig: () => Promise<GatewayConfigRecord>
  saveConfig: (config: GatewayConfigRecord) => Promise<{ ok: boolean }>
}
...
export function getConfigDisplayLanguage(config: GatewayConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: GatewayConfigRecord, locale: Locale): GatewayConfigRecord {
