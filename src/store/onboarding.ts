...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/nexus'
...
const CONFIGURED_CACHE_KEY = 'nexus-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'nexus-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Nexus Agent is ready', message: `${provider} connected.` })
}
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Nexus Agent still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Nexus Agent still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.nexusAgent?.openExternal) {
    try {
      await window.nexusAgent.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Nexus Agent still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Nexus Agent still cannot reach ${url}.` }
    }
